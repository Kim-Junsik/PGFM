...
from .heads import HurdleHead, MSEHead
...
class GeneWiseMSEHead(MSEHead):
    """Per-gene readout. Loss and point estimate are inherited unchanged."""

    def __init__(self, d_value: int, n_genes: int):
        nn.Module.__init__(self)
        self.weight = nn.Parameter(torch.randn(d_value) * 0.02)
        self.bias = nn.Parameter(torch.zeros(n_genes))

    def forward(self, h: torch.Tensor) -> dict[str, torch.Tensor]:
        return {"mean": torch.einsum("bgd,d->bg", h, self.weight) + self.bias}
...
class PCABBackbone(BaseBackbone):
    def __init__(self, config: dict, n_genes: int, prior: np.ndarray):
        ...
        self.from_latent = mlp_stack([self.latent_dim + d_key, 2 * d_value, d_value],
        ...
        if head_kind == "hurdle":
            ...
            self.head = GeneWiseMSEHead(d_value, n_genes)
        else:
    ...
    def decode(self, z: torch.Tensor) -> dict[str, torch.Tensor]:
        batch = z.shape[0]
        joined = torch.cat([
            z.unsqueeze(1).expand(batch, self.n_tokens, -1),
            self.token_embedding.unsqueeze(0).expand(batch, -1, -1)], dim=-1)
