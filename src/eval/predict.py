...
from ..models.heads import ZINBHead
from . import metrics


def _head_aux(vae, x: torch.Tensor) -> dict:
    """The zinb head needs a library size; every other head ignores this."""
    if not isinstance(getattr(vae, "head", None), ZINBHead):
        return {}
    _, library = recover_counts(x)
...
def evaluate_model(vae, field, data, stats, folds, method, config,
    ...
    return {
        ...
        "resid_R2_mean": float(np.nanmean(per_double)) if edists else float("nan"),
