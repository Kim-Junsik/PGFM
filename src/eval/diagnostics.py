...
them are indistinguishable in the summary table:
  ...
  a closed gate     `commutator` reduces numerically to `additive`, so the
                    ablation compares a model against itself and reports a
                    null result that is really a missing measurement.
...
from ..models.flow import LieCFMField, integrate
...
@torch.no_grad()
def measure_interaction(vae, field, data, doubles: list[str], config, rng,
                        device: str, n_cells: int) -> list[dict]:
    """The size of the interaction term relative to the additive part it corrects.

    Measured ALONG THE REAL TRAJECTORY, not on a grid: the field is only ever
    queried at the points RK4 visits, and a term that is large somewhere the
    integrator never goes changes no prediction.

    The interaction is read off as (composed field - additive sum) rather than by
    calling the gate directly, so commutator and free_mlp need no special casing
    and the number is exactly what enters the velocity.
    """
    n_steps = config["train"]["n_integration_steps"]
    kind = field.interaction_kind
    rows = []
    for condition in doubles:
        ...
        additive_norms, interaction_norms = [], []
        step_size = 1.0 / n_steps
        for step in range(n_steps):
            t0 = torch.full((1,), step * step_size, device=z.device, dtype=z.dtype)
            additive = field.generator(z, t0, perturbations[0])
            for pert in perturbations[1:]:
                additive = additive + field.generator(z, t0, pert)
            total = field(z, t0, perturbations)
            additive_norms.append(float(additive.norm(dim=-1).mean()))
            interaction_norms.append(float((total - additive).norm(dim=-1).mean()))

            # Advance with the same RK4 as flow.integrate, so the points sampled
            # above are the ones inference actually visits. k1 is `total`, which
            # was already computed for the measurement.
            half, full = t0 + 0.5 * step_size, t0 + step_size
            k2 = field(z + 0.5 * step_size * total, half, perturbations)
            k3 = field(z + 0.5 * step_size * k2, half, perturbations)
            k4 = field(z + step_size * k3, full, perturbations)
            z = z + (step_size / 6.0) * (total + 2 * k2 + 2 * k3 + k4)

        a, b = perturbations
        additive_mean = float(np.mean(additive_norms))
        interaction_mean = float(np.mean(interaction_norms))
        rows.append({
            "condition": condition,
            # Lambda_ab is only defined for the commutator arm; free_mlp has no
            # scalar coefficient to report, just the term it produces.
            "lambda": float(field.gate(a, b)) if kind == "commutator" else float("nan"),
            "additive_norm": additive_mean,
            "interaction_norm": interaction_mean,
            "relative": interaction_mean / additive_mean if additive_mean else float("nan"),
        })
    return rows
